...
var _ = Describe("SharedSchemaReader", func() {
	...
	Context("ListSharedSchemas", func() {
		It("should paginate without duplicates or gaps", func() {
			ctx := context.Background()

			// Write several shared schemas with unique prefixed IDs
			for i := 0; i < 5; i++ {
				version := xid.New().String()
				schemaID := "list-test-" + string(rune('a'+i))
				defs := []storage.SharedSchemaDefinition{
			...
			pages := 0
			for {
				page, ct, err := sharedSchemaReader.ListSharedSchemas(ctx, database.NewPagination(database.Size(2), database.Token(token)))
				Expect(err).ShouldNot(HaveOccurred())
				for _, item := range page {
					Expect(seen[item.SharedSchemaId]).Should(BeFalse(), "duplicate: "+item.SharedSchemaId)
					seen[item.SharedSchemaId] = true
				}
				pages++
				token = ct.String()
				if token == "" {
				...
				Expect(pages).Should(BeNumerically("<", 100), "infinite pagination loop")
			}

			// All 5 schemas we wrote must be present
			for i := 0; i < 5; i++ {
				id := "list-test-" + string(rune('a'+i))
				Expect(seen[id]).Should(BeTrue(), "missing: "+id)
			}
		})
	})
